            ...
            <div class="state">
                <svg class="svg svg-icon" viewBox="0 0 14 14">
                    ...
                    <path
                        ...
                        id="Path" fillOpacity="0.01" fill="#000000"></path>
...
async function getSongPool(accessToken) {
  ...
  for (let playlist of selectedPlaylists) {
    let songsInPlaylist = await getSongsInPlaylist(
      playlistsMap[playlist],
      accessToken
    );
...
async function createNewPlaylist(accessToken) {
  ...
  if (userInputs["artistsFilteringEnabled"]) {
    newPlaylist = getSongsMatchingArtist(
      userInputs["desiredArtists"],
      newPlaylist
    );
  }

  newPlaylist = await getSongsMatchingFilters(
    userInputs,
    newPlaylist,
    accessToken
  );
  ...
  let createPlaylistURL =
    "https://api.spotify.com/v1/users/" + state["userID"] + "/playlists";
  ...
  for (
    let songIndex = 0;
    songIndex < newPlaylistURIs.length;
    songIndex += 100
  ) {
    /* Can only add 100 songs per request */
    addTracksToPlaylist(
      playlist,
      newPlaylistURIs.slice(songIndex, songIndex + 100),
      accessToken
    );
  }
  toggleSpinner();
